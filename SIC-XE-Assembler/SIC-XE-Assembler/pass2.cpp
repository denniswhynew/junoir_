...
int main()
{
	...
	{
		error_flag = 1;
		error << "output File has no start!" << endl;
	}
	while (true)
	{
		input(a);
...
void assemble(string a[])
{
	...
	if (a[2] == "EQU")
	{
		...
		else
		{
			...
			for (int h = k + 1; h < s.length(); h++)
				s2 += s[h];
			if (add_or_sub)
				SymbleTable[a[1]].address = toHex(toDec(SymbleTable[s1].address) + toDec(SymbleTable[s2].address));
			else
				SymbleTable[a[1]].address = toHex(toDec(SymbleTable[s1].address) - toDec(SymbleTable[s2].address));
		}
		return;
	...
	if (a[2] == "BASE")
	{
		base = toDec(SymbleTable[a[3]].address);
		return;
	}
	imm = ind = false;
	...
	if (a[2] == "LDB")
	{
		base = toDec(SymbleTable[a[3]].address) + toDec(Block[SymbleTable[a[3]].block].address);
	}
}
...
string gen_code(string a[])
{
	...
	if (format == 2)
	{
		...
		if (a[3].length() == 3)
		{
			...
			if (isdigit(a[3][2]))  ob2 = ob2 + _toHex(a[3][2] - 1);
			else
			{
				ob2 = ob2 + _toHex(reg_num(a[3][2]));
			}
		}
		else  //a[3].length==1
		{
	...
	if (format == 3)
	{
		...
		operand_addr = toHex(toDec(SymbleTable[a[3]].address) + toDec(Block[SymbleTable[a[3]].block].address));
