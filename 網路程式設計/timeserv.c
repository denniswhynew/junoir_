...
int main(int argc, char **argv)
{
	...
	int a1, a2, a3, a4, i;					/* Components of address in xxx.xxx.xxx.xxx form */
	int client_length;					/* Length of client struct */
	int bytes_received;					/* Bytes received from client */
	SOCKET sd;							/* Socket descriptor of server */
	struct sockaddr_in server;			/* Information about the server */
	//struct sockaddr_in server2;
	struct sockaddr_in client1;			/* Information about the client */
	...
	//memset((void *)&server2, '\0', sizeof(struct sockaddr_in));
	...
	while (1)
	{
		...
		bytes_received = recvfrom(sd, buffer, BUFFER_SIZE, 0, (struct sockaddr *)&client1, &client_length);
		if (bytes_received < 0)
		{
		...
		bytes_received = recvfrom(sd, buffer, BUFFER_SIZE, 0, (struct sockaddr *)&client2, &client_length);
		if (bytes_received < 0)
		{
			fprintf(stderr, "Could not receive datagram.\n");
			closesocket(sd);
		...
		if (strcmp(buffer, "GET TIME\r\n") == 0)
		{
			...
			for(i = 1; i < 1001; i++)
            {
                ...
                /* Send data back */
                if (sendto(sd, (char *)&current_time, (int)sizeof(current_time), 0, (struct sockaddr *)&client1, client_length) != (int)sizeof(current_time))
                {
                ...
                if (sendto(sd, (char *)&current_time, (int)sizeof(current_time), 0, (struct sockaddr *)&client2, client_length) != (int)sizeof(current_time))
                {
                    fprintf(stderr, "Error sending datagram.\n");
                    closesocket(sd);
                    WSACleanup();
